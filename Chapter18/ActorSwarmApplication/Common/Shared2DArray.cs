...
{
    [DataContract]
    public class Shared2DArray<T> where T : IComparable
    {
        private int mSize;
        private T[,] mArray;
        ...
        public void ResolveConflictsAndCommit(Action<IProposal> callback)
        {
            ...
            {
                var otherPropoals = from p in mProposals
                                    where p.NewX == proposal.NewX && p.NewY == proposal.NewY
                                    && p.OldX != proposal.OldX && p.OldY != proposal.OldY
                                    select p;
                if (otherPropoals.Count() == 0)
                {
